...
工会公共网页：任何人打开链接都能看**总览**与**团队编组**（只读），并用顶栏切换主/附属俱乐部；**成员档案 / DKP / 联赛 / 设置**需管理员登录。论坛已关闭。
...
- 访客：不用登录可看总览、团队编组（只读），可切换俱乐部；成员档案等需管理员登录
...
| 身份        | 能力                |
| --------- | ----------------- |
| 任何人（有链接）  | 看总览、团队编组（只读）；切换主/附属俱乐部 |
| 管理员（账号密码） | 成员档案、DKP、联赛、设置、改俱乐部名、编组调整、导入备份 |
